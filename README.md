...
- `User Interface`：Telegram、PowerShell TUI 和飞书机器人是三条交互入口，分别面向远程移动端、本机终端和飞书组织内协作场景。
- `Bot Layer`：`telegram_bot.py` 负责 Telegram 消息路由、owner 权限判断、命令处理、媒体保存、ASR 转写、异常处理和回复；`tui.py` 负责本地输入循环、命令处理和终端输出；`feishu_bot.py` 负责飞书长连接事件、文本消息解析、白名单校验和回复。
...
```powershell
python -m garveyclaw.feishu_bot
```
